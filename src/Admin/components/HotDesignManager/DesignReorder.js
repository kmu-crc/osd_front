...
const Container = styled.div`
    display: flex;
`;
class DesignReorder extends Component {
    state = {
        job: false,
    ...
    onApply = () => {
        ...
        special.forEach((item, i) => {
            //insert
            if (item.order === null) {
                job.push({ uid: item.uid, data: { order: i, type: "insert" } })
                item.order = i;
            }
            //update
        ...
        return (
            <div>
                ...
                <div style={{ textAlign: "center", fontSize: "20px", width: "100%" }}><br />
                    {this.state.job
                        ? <button style={{ width: "25%" }} onClick={this.onApply} enable="enable">변경내용적용하기</button>
                        : <button style={{ width: "25%" }} onClick={this.onApply} disabled="disabled">변경내용적용하기</button>}
