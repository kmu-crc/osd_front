...
//                         <DesignExplanation>
//                           <TextFormat txt={designDetail.explanation} lines={2} />
//                         </DesignExplanation>
//                       </DesignInfoCard>
//                     </Grid.Column>
//                   </Grid.Row>
...
//                         <InfoItem>
//                           <h3>작성자</h3>
//                           <Link to={`/designerDetail/${designDetail.user_id}`}><TextFormat txt={designDetail.userName} chars={16} /></Link>
//                         </InfoItem>
//                         <InfoItem>
//                           <h3>카테고리</h3>
//                           <TextFormat txt={designDetail.categoryName ? designDetail.categoryName : "전체"} chars={16} />
//                         </InfoItem>
//                         <InfoItem>
//                           <h3>멤버</h3>
...
//                         <InfoItem>
//                           <h3>가격</h3>
//                           <div style={{ fontSize: "24px", fontWeight: "bold" }}>{MoneyStyle(designDetail.price)}</div>
//                         </InfoItem>
...
import TextFormat from 'modules/TextFormat';
import DesignDetailViewContainer from 'containers/Designs/DesignDetailViewContainer';
// import { Design } from 'redux/modules/design';
...
class DesignDetail extends Component {
  ...
  render() {
    console.log("로그로그", this.props, this.props.DesignDetail.img);
    const { DesignDetail } = this.props;
    const reviewComment = DesignDetail.reviews && DesignDetail.reviews.map((item, index) => {
      return <ReviewComment key={index} img={item.img} who={item.who} name={item.user_id} comment={item.comment} option="" />;
    })
    const optionList = DesignDetail.options && DesignDetail.options.map((item, index) => {
      return <ProductOption key={index} type={item.type} name={item.name} options={item.options} />;
    })
    const optionSmallImgList = DesignDetail.img && DesignDetail.img.map((item, index) => {
      return <SmallImage onClick={() => this.onChangeBigImage(index)} imageURL={item.m_img} />
    })
          ...
          <div className="content_box">
            <div className="left_side_box">
              <div className="image_box">
                <div className="big_image_box">
                  <BigImage imageURL={DesignDetail.img && DesignDetail.img[this.state.select_image].l_img} />
                </div>
            ...
            <div className="right_side_box">
              <div className="info_box">
                <div className="product_name">{DesignDetail.title}</div>
                <div className="product_explain"><TextFormat txt={DesignDetail.explanation} single /></div>
                <div className="product_price">₩{DesignDetail.price || 0}</div>
                <div className="product_option">
                  {optionList}
                </div>
                <div className="product_stock">
                  <div className="amount_label">수량</div>
                  <div><FormText value="1"></FormText></div>
                  <div className="stock_label">(재고:{DesignDetail.amount})</div>
                </div>
              ...
              <div className="delivery_box">
                ...
                {DesignDetail && DesignDetail.delivery ?
                  <div className="delivery_explain">
                    배송사: {DesignDetail.delivery.company}<br />
                    배송기간: {DesignDetail.delivery.days}<br />
                    배송료: {DesignDetail.delivery.cost}<br />
                    반품: 반품료는 {DesignDetail.delivery.cost}원이며 반품시 택배 박스와 함께 <br />
                    현금을 동봉해주시기 바랍니다.<br />
                  </div> : "배송정보가 없습니다."}
              </div>
