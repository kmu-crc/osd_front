...
class CardModalMobile extends Component {
  ...
  handleCancel = async (obj) => {
    if (
      ...
      if (
        !(await confirm(
          "작업중인 데이터는 저장되지 않습니다. 그래도 하시겠습니까?",
          "예",
          "아니오"
        ))
      ) {
  ...
  handleClosed = async (obj) => {
    if (this.state.edit) {
      if (this.state.title !== this.props.card.title) {
        if (
          await confirm(
            "제목이 변경되었습니다, 저장하지 않고 수정모드를 종료하시겠습니까?",
            "예",
            "아니오"
          )
        ) {
  ...
  handleHeaderSubmit = (formData) => {
    ...
    ValidationGroup(this.state, false)
      .then(async (data) => {
        ...
        // return;
        await this.props
          .UpdateCardSourceRequest(pack, this.props.card.uid, this.props.token)
          .then(
            this.props.UpdateDesignTime(this.props.designId, this.props.token)
          )
  ...
  onClose = async () => {
    if (this.state.edit) {
      if (this.state.title !== this.props.card.title) {
        if (
          await confirm(
            "제목이 변경되었습니다, 저장하지 않고 창을 닫으시겠습니까?",
            "예",
            "아니오"
          )
        ) {
      ...
      if (this.state.content !== this.props.card.content) {
        if (
          await confirm(
            "설명이 변경되었습니다, 저장하지 않고 창을 닫으시겠습니까?",
            "예",
            "아니오"
          )
        ) {
      ...
      if (this.state.isEdited) {
        if (
          await confirm(
            "내용이 변경되었습니다, 저장하지 않고 창을 닫으시겠습니까?",
            "예",
            "아니오"
          )
        ) {
  ...
  render() {
    const imgURL =
      (this.props.card &&
        this.props.card.first_img &&
        this.props.card.first_img.l_img) ||
      null;
        ...
        <div className="content-wrapper">
              ...
              <div className="card-header-second">
                ...
                <div className="card-header-third">
                  ...
                  <div className="update-time">
                    (업데이트&nbsp;:&nbsp;{DateFormat(card.update_time)})
                  </div>
                </div>
              </div>
            </div>
            ...
            <EditCardHeaderContainer>
              {card.user_id ===
                ...
                <div className="private-box-toggle">
                  <div
                    className={`icon-wrapper ${this.state.private ? "lock" : "unlock"
                      }`}
                    onClick={() =>
          ...
          <CommentWrapper>
            ...
            <div className="comment-body">
              <CardComment
                designId={this.props.design_id}
                cardId={this.props.card.uid}
