...
import { InvitedUserRequest, CancelInvitedUserRequest } from "redux/modules/group";
...
class Room extends React.Component {
	...
	render() {
		console.log("grouproom:", this.props);
		const { group, peers, me, roomClient, consumers, /*  room, onRoomLinkCopy */ } = this.props;
		const bg = (group && group.img && group.img.l_img) || nobg;
		...
		return (<React.Fragment>
			...
			<RoomDiv h={h || window.innerHeight}>
				...
				<InviteModal open={invite} onClose={() => this.setState({ invite: false })}>
					...
					<span style={{ width: "max-content", marginLeft: "auto", marginRight: "10px" }}>(그룹 맴버가 아닌 오픈소스사이트 사용자를 회의에 초대합니다.)</span>
					...
					<div className="button-bar" style={{ display: "flex", flexDirection: "row", justifyContent: "flex-end" }}>
						<div style={{ cursor: "pointer", marginRight: "25px", color: "red", fontSize: "1.5rem", fontWeight: "500", width: "max-content" }}
							onClick={() => {
								this.state.selected &&
									this.state.selected.length > 0 &&
									this.state.selected.forEach(async mem => {
										// alert(mem.uid); return;
										try {
											InvitedUserRequest(group.uid, this.props.token, mem.uid) //{ to_user_id: mem.uid })
										} catch (e) {
					...
					<div style={{ display: "flex", flexDirection: "row" }}>
						...
						<div>
							...
							<div className='btn exit' onClick={() => {
								if (peers.length === 0) {
									CancelInvitedUserRequest(this.props.group.uid, this.props.token)
								}
